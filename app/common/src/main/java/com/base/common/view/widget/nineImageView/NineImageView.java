...
public class NineImageView extends ViewGroup {
    ...
    private List<GlideImageView> imageViews;
    ...
    private void init(Context context, AttributeSet attrs) {
        int WIDTH = DensityUtil.getScreenWidthPixels() - DensityUtil.dp2px(15 * 2);
    ...
    private void layoutChildrenView() {
        ...
        for (int i = 0; i < childrenCount; i++) {
            GlideImageView imageView = (GlideImageView) getChildAt(i);
    ...
    private void loadImage(GlideImageView imageView, ImageAttr attr, final int count) {
...
//        ImageLoaderUtils.loadImage(imageView, url, false);
        ...
        RequestOptions requestOptions = imageLoader.requestOptions(R.color.C_333333).centerCrop().skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.RESOURCE).override(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL);

        imageLoader.requestBuilder(url, null, requestOptions).transition(DrawableTransitionOptions.withCrossFade()).listener(new RequestListener<Drawable>() {
            @Override
    ...
    public void setAdapter(@NonNull NineImageViewAdapter adapter) {
        ...
        if (imageAttrs == null) {
            for (int i = 0; i < imageCount; i++) {
                GlideImageView imageView = getImageView(i);
    ...
    private GlideImageView getImageView(final int position) {
        GlideImageView imageView = null;
