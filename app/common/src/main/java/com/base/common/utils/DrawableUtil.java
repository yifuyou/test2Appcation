...
public class DrawableUtil {
    ...
    public static Bitmap getAssetsBitmap(String filename) {
        return UIUtils.getAssetsImage(filename);
    }
    ...
    public static Bitmap getBitmap(@DrawableRes int resId, int x, int y) {
        Drawable drawable = getDrawable(resId);
        return getBitmap(drawable, x, y);
    }

    public static Bitmap getBitmap(Drawable drawable, int x, int y) {

        drawable.setBounds(0, 0, x, y);
        // 取 drawable 的颜色格式
        Bitmap.Config config = drawable.getOpacity() != PixelFormat.OPAQUE ? Bitmap.Config.ARGB_8888 : Bitmap.Config.RGB_565;
        Bitmap bit_bg = createBitmap(x, y, config);
        //将背景绘制到bit_bg里
        Canvas can_bg = new Canvas(bit_bg);
        drawable.draw(can_bg);
        return bit_bg;
    }
...
//    }
     ...
     * @return
     */
    public static String bitmpToString(@NonNull Bitmap bitmap) {
        byte[] bytes = bitmapToByte(bitmap, true);
        return "data:images/png;base64," + Base64.getEncoder().encodeToString(bytes);
    }

    /*将位图转换为字节 needRecycle 是否释放*/
    public static byte[] bitmapToByte(@NonNull Bitmap bmp, boolean needRecycle) {
     ...
     * 获取drawable
     */
    public static ColorDrawable getColorDrawableRes(@ColorRes int resId) {
        return new ColorDrawable(ColorUtils.getColor(resId));
    }

    public static ColorDrawable getColorDrawable(@ColorInt int resId) {
        return new ColorDrawable(resId);
    }
...
//        canvas.restore();
    ...
    public static byte[] binaryzationBitmap(byte[] input, int tmp) {
        ...
        if (bitmap != null) {
            ...
            if (res != null) return bitmapToByte(res, true);
        }
        return null;
    }
...
//        view.layout(0, 0, w, h);
    ...
    /**
     * View转换为Bitmap图片 真实view高度 比如NestedScrollView长图
     *
     * @param view 未显示的
     * @return
     */
    public static Bitmap convertViewToBitmap(View view, int width) {
        if (view == null) return null;
        return convertViewToBitmap(view, width, view.getHeight());
    }
    ...
    /**
     * @param view
     * @param subtractHeight 减去的高度
     * @return
     */
    public static Bitmap convertRealViewToBitmap(View view, int subtractHeight) {
        if (view == null) return null;
         ...
         */
        view.measure(measuredWidth, measuredHeight);
        view.layout(0, 0, view.getMeasuredWidth(), view.getMeasuredHeight() - subtractHeight);
    ...
    public static Bitmap getDesignatedAreaBitmap(View view) {
        View screenView = AppManager.getInstance().getTopActivity().getWindow().getDecorView();
        ...
        if (bitmap != null) {
            ...
            view.getLocationInWindow(viewLocationArray);
