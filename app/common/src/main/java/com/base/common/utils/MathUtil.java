...
public class MathUtil {
     ...
     * @return
     */
    public static Point getCoordinate(Point origin, Point pos, int strokeWidth) {
        //Math.atan2  计算出来是一个弧度值，转换成角度必须乘以  180/PI、
        //  double angle = Math.atan2(tan[1], tan[0]) * 180 / Math.PI;

        if (pos.x == origin.x) {
            if (pos.y > origin.y) {
                return new Point(pos.x, origin.y + strokeWidth);
            } else {
                return new Point(pos.x, origin.y - strokeWidth);
            }
        ...
        double tans = (double) (pos.y - origin.y) / (double) (pos.x - origin.x);
