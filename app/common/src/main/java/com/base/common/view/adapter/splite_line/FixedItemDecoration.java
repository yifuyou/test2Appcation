...
public class FixedItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private int fixed_Posi = -1;//当前悬浮的位置

    private int typeFixed = -1;

    private boolean fixedStatus = false;//当前悬浮状态
    ...
    private boolean hasStickHeader(int position) {
        ...
        return adapter.getItemViewType(position) == typeFixed;
    }

    public int getFixedViewPosition(int position) {
        if (position == -1 || adapter == null) return -1;
        for (int i = position; i >= 0; i--) {
            if (adapter.getItemViewType(i) == typeFixed) return i;
        }
        return -1;
    }

    @Override
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, final @NonNull RecyclerView.State state) {
...
//            position = parent.getChildAdapterPosition(firstView);
        ...
        //获取需要悬浮的position
        int pos = getFixedViewPosition(position);
        if (pos == -1) {
            if (fixed_Posi > -1 && fixedStatus) {
                fixed_Posi = -1;
                adapter.notifyItemChanged(fixed_Posi);
            }
            fixedStatus = false;
            return;
        } else {
        ...
        //创建需要悬浮的view
        if (viewHolder == null) {
            viewHolder = adapter.createViewHolder(parent, typeFixed);
        }

        if (pos != fixed_Posi || pos == 0) {
            fixed_Posi = pos;
            //绑定数据
            adapter.onBindViewHolder(viewHolder, fixed_Posi);
        }
        ...
        if (isNeedToBindAndMeasureView(parent, viewHolder.itemView, mIsHorizontal, position)) {
            /** 测量工作必须在这里处理,因为默认是布局处理的布局是wrap_content,需要设置数据之后再进行测量计算工作
             * 否则如果布局中某些view是wrap_content,当不存在数据时该view大小将为0,即无法显示
             * */
            measureHeaderView(parent, viewHolder.itemView, mIsHorizontal);
        }
                ...
                switch (e.getAction()) {
                    case MotionEvent.ACTION_UP:
                        ...
                        if (x <= rect.right && y <= rect.bottom) {
                            if (viewHolder != null) {
                                //查找当前xy位置的view
                                View view = ViewUtils.getViewByPosition(viewHolder.itemView, point, x, y);
                                if (view != null) {
                                    View onclickView = findOnclickView(view);
                                    if (onclickView != null && onclickView.hasOnClickListeners()) {
                                        onclickView.callOnClick();
                                    }
                                }
                            }
                            return true;
    ...
    private View findOnclickView(@NonNull View view) {
        if (view == null) {
            return null;
        }
        ...
        //查找父类
            ...
            if (view.getParent() != viewHolder.itemView) {
                return findOnclickView((View) view.getParent());
            } else {
    ...
    public void calculateFixedRect(Rect mOutRect, Point mOutPoint) {
        if (parent == null || viewHolder == null) return;
        //计算当前headerView需要绘制的区域
        this.calculateViewDrawRect(mOutRect, parent, viewHolder.itemView, mIsHorizontal);
...
//        int drawTop = parent.getTop() + parent.getPaddingTop();
     ...
     * @return
     */
    protected Point calculateViewDrawRectInflunceByOtherView(Point outPoint, int childPosition, Rect normalRect, RecyclerView parent, RecyclerView.Adapter adapter, boolean isHorizontal) {
        ...
        if (headerView != null) {
            ...
            if (isHorizontal) {
                ...
                if (headerView.getTop() < rect.bottom && headerView.getTop() > rect.top) {
                    //如果查找得到的headerView已经在替换当前的stick viewHolder
