...
public class CircleProgressView extends ProgressBar {
    ...
    private int mReachBarSize = DensityUtil.dp2px(2); // 未完成进度条大小
    private int mNormalBarSize = DensityUtil.dp2px(2); // 未完成进度条大小
    private int mReachBarColor = Color.parseColor("#108ee9"); // 已完成进度颜色
    private int mNormalBarColor = Color.parseColor("#FFD3D6DA"); // 未完成进度颜色
    private int mTextSize = DensityUtil.sp2px(14); // 进度值字体大小
    ...
    private int mOuterSize = DensityUtil.dp2px(1); // 外层圆环宽度
    ...
    private void drawNormalCircle(Canvas canvas) {
        ...
        // 绘制内部圆形背景色
        if (needDrawInnerBackground) {
            canvas.drawCircle(0, 0, mRadius - Math.min(mReachBarSize, mNormalBarSize) / 2, mInnerBackgroundPaint);
        }
        // 绘制文字
        if (mTextVisible) {
            String text = mTextPrefix + getProgress() + mTextSuffix;
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = DensityUtil.dp2px(reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = DensityUtil.dp2px(normalBarSize);
        invalidate();
    }
    ...
    public void setReachBarColor(@ColorInt int reachBarColor) {
    ...
    public void setNormalBarColor(@ColorInt int normalBarColor) {
    ...
    public void setTextSize(int textSize) {
        mTextSize = DensityUtil.sp2px(textSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = DensityUtil.dp2px(radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = DensityUtil.dp2px(innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = DensityUtil.dp2px(outerSize);
        invalidate();
    }
