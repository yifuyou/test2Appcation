...
import com.base.zbar.databinding.ActivityQrBinding;
...
public class QRActivity extends BaseTitleActivity<ActivityQrBinding, BaseViewModel> implements View.OnClickListener, SensorEventListener {
...
//        }
    ...
    public void initView() {
        setTitle(options.getTitle_text());
        ...
        iv_flash = (ImageView) findViewById(cn.bertsir.zbar.R.id.iv_flash);
//        iv_flash.setOnClickListener(this);
        binding.llFlashlight.setOnClickListener(this);
        iv_flash.setImageResource(options.getLightImageRes());


        iv_album = (ImageView) findViewById(cn.bertsir.zbar.R.id.iv_album);
        iv_album.setOnClickListener(this);
        iv_album.setImageResource(options.getAblumImageRes());

        tv_des = (TextView) findViewById(cn.bertsir.zbar.R.id.tv_des);

        vsb_zoom = (VerticalSeekBar) findViewById(cn.bertsir.zbar.R.id.vsb_zoom);
    ...
    public void onClick(View v) {
        if (v.getId() == cn.bertsir.zbar.R.id.iv_album) {
            fromAlbum();
        } else if (v.getId() == R.id.llFlashlight) {
            if (cp != null) {
                cp.setFlash();
            }
        } else if (v.getId() == cn.bertsir.zbar.R.id.mo_scanner_back) {
    ...
    private void recognitionLocation(Uri uri) {
        ...
        textDialog.setText(UIUtils.getString(R.string.qr_scan_2));
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    if (TextUtils.isEmpty(imagePath)) {
                        Toast.makeText(getApplicationContext(), UIUtils.getString(R.string.qr_scan_3), Toast.LENGTH_SHORT).show();
                        return;
                    }
                    //优先使用zbar识别一次二维码
                                ...
                                if (!TextUtils.isEmpty(qrcontent)) {
                                    ...
                                    try {
                                        ...
                                        if (!TextUtils.isEmpty(barcontent)) {
                                            ...
                                            Toast.makeText(getApplicationContext(), UIUtils.getString(R.string.qr_scan_4), Toast.LENGTH_SHORT).show();
                                            closeProgressDialog();
                                        }
                                    } catch (Exception e) {
                                        Toast.makeText(getApplicationContext(), UIUtils.getString(R.string.qr_scan_5), Toast.LENGTH_SHORT).show();
                                        closeProgressDialog();
                                        e.printStackTrace();
                                    }
    ...
    public TextView showProgressDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this, cn.bertsir.zbar.R.style.AlertDialogStyle);
        builder.setCancelable(false);
        View view = View.inflate(this, cn.bertsir.zbar.R.layout.dialog_loading, null);
        builder.setView(view);
        ProgressBar pb_loading = (ProgressBar) view.findViewById(cn.bertsir.zbar.R.id.pb_loading);
        TextView tv_hint = (TextView) view.findViewById(cn.bertsir.zbar.R.id.tv_hint);
        if (Build.VERSION.SDK_INT >= 23) {
            pb_loading.setIndeterminateTintList(getColorStateList(cn.bertsir.zbar.R.color.dialog_pro_color));
        }
