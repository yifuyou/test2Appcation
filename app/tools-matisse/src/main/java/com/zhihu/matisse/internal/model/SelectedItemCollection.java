...
//    }
    ...
    public List<Uri> asListOfUri() {
        ...
        for (Item item : mItems) {
            ...
            uris.add(item.uriEditer == null ? item.getContentUri() : item.uriEditer);
        }
        return uris;
    }
    ...
    public List<String> asListOfString() {
        ...
        for (Item item : mItems) {
            item.uriEditer = ImageEditUtils.getInstance().get(item.getContentUri().getEncodedPath());
            paths.add(PathUtils.getPath(mContext, item.uriEditer == null ? item.getContentUri() : item.uriEditer));
        }
        return paths;
    }
