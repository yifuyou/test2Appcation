...
public abstract class BasePreviewActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //如果是图片编辑
        if (ImageEditUtils.getInstance().getImageEditInterface() != null) {
            mButtonBack.setText(ImageEditUtils.getInstance().getImageEditInterface().editString());
            ImageEditUtils.getInstance().setImageEditReturnInterface(new ImageEditUtils.ImageEditReturnInterface() {
                @Override
                public void editImageReturn(File mImageFile) {
                    //替换掉里面的地址
                    Item item = mAdapter.getMediaItem(mPager.getCurrentItem());
                    item.uriEditer = Uri.fromFile(mImageFile);

                    ImageEditUtils.getInstance().put(item.getContentUri().getEncodedPath(), item.uriEditer);
                    //更新显示
                    PreviewPagerAdapter adapter = (PreviewPagerAdapter) mPager.getAdapter();
                    adapter.getItemViews(mPager.getCurrentItem()).loadImage(item);
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (ImageEditUtils.getInstance().getImageEditInterface() != null) {
            ImageEditUtils.getInstance().getImageEditInterface().onActivityResult(requestCode, resultCode, data);
        }
    }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.button_back) {
            if (ImageEditUtils.getInstance().getImageEditInterface() != null) {
                Item item = mAdapter.getMediaItem(mPager.getCurrentItem());
                if (item.isGif() || item.isVideo()) return;
                ImageEditUtils.getInstance().getImageEditInterface().editOnclickListener(v, item.uriEditer == null ? item.uri : item.uriEditer);
            } else {
                onBackPressed();
            }
